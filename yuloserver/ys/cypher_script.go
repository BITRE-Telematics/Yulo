...
func cypherScript() map[string]string {
	...
	script["stops"] = `
		...
		LOAD CSV WITH HEADERS FROM 'file:///StopsOut%s.csv' AS row
	...
	script["obvs"] = `
		...
		LOAD CSV WITH HEADERS FROM 'file:///mergedPBF%s.csv' AS row
	...
	script["trips"] = `
		USING PERIODIC COMMIT 1000
		LOAD CSV WITH HEADERS FROM 'file:///Tripframe%s.csv' AS row
		MATCH (trip:Trip{
		  id: row.Trip})
