...
func stopwrite(stop processedStop, id string, i int) {
	...
	stopquery := `
			    ...
			    FOREACH (ignoreMe IN CASE WHEN $LOC <> "" THEN [1] ELSE [] END |
		        MERGE (addr:Location{
				  id: $LOC
				})
...
func obvswrite(obv processedObv, id string, trip string, onDone func()) {
	defer onDone()
	var o_type string
...
func obvwritesingle(obv writeObv, id string, i int) {
	...
	res, err := session.Run(obvquery, parameters)
	if err != nil && i < 400 {
		if i > 10 {
...
func tripwrite(trip processedTrip, i int) {
	...
	res, err := session.Run(tripquery, parameters)
	if err != nil && i < 400 {
		if i > 50 {
			fmt.Println(err, trip.Trip, i)
		}
		time.Sleep(time.Millisecond * 200)
		go tripwrite(trip, i+1)
	}
...
func tripswrite(trips []processedTrip, id string) {
	...
	for _, t := range trips {
		for _, o := range t.Obvs {
			wg.Add(1)
			go obvswrite(o, id, t.Trip, func() { wg.Done() })
			//fmt.Print(o)
		}
		tripwrite(t, 1)
