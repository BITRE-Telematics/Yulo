    ...
    public BaseRoad next() throws SourceException {
        if (result_set == null) {
            ...
            String query = "SELECT gid,osm_id,class_id,source,target,"
                    ...
                    + "priority, ST_AsBinary(geom) as geom, sa2, gcc FROM " + table + where + ";"; //ad sa2, gcc
        ...
        try {
            ...
            do {
                ...
                float priority = (float) config.get(classId).one().doubleValue(); //add sa2, gcc
                ...
                String sa2 = result_set.getString(12);
                String gcc = result_set.getString(13);

                road = new BaseRoad(gid, source, target, osmId, reverse >= 0 ? false : true,
                        classId, priority, maxspeedForward, maxspeedBackward, length, wkb, sa2, gcc);
