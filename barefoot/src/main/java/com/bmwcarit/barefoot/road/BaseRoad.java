...
public class BaseRoad implements Serializable {
    ...
    private final double med_azi;
    private final String sa2;
    ...
    public BaseRoad(long id, long source, long target, long refid, boolean oneway, short type,
            ...
            Polyline geometry, String sa2, String gcc) {
        this.id = id;
        this.source = source;
        this.target = target;
        this.refid = refid;
        ...
        double azimuths[] = new double[11];
                for(int i=0; i<11; i++){
                  double prop = i / 10;
                  azimuths[i] = spatial.azimuth(geometry, prop);
                }
        Arrays.sort(azimuths);
        double medazi = azimuths[5];
        this.med_azi = medazi;
        this.sa2 = sa2;
        this.gcc = gcc;
    }
    ...
    public BaseRoad(long id, long source, long target, long osmId, boolean oneway, short type,
        ...
        Polyline geometry_ = (Polyline) OperatorImportFromWkb.local().execute(WkbImportFlags.wkbImportDefaults,
                 Type.Polyline, ByteBuffer.wrap(geometry), null);
        double azimuths[] = new double[11];
                for(int i=0; i<11; i++){
                  double prop = i / 10;
                  azimuths[i] = spatial.azimuth(geometry_, prop);
                }
        Arrays.sort(azimuths);
        double medazi = azimuths[5];
        this.med_azi = medazi;
    ...
    // /**
    //  * Gets median azimuth of road segment.
    //  *
    //  * @return median azimuth.
    //  */
    public double med_azi() {
        return med_azi;
    }


    // /**
    //  * Gets sa2 of road segment.
    //  *
    //  * @return sa2.
    //  */
    public String sa2() {
