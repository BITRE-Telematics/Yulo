...
parser.add_option("--seg_sa2s", dest="seg_sa2s",
                  help="JSON files of segment geograpy (Sa2s)")
...
with open(options.seg_sa2s, 'r') as f:
  seg_sa2s = json.load(f)
...
bfmap.ways2bfmap(options.source_host, options.source_port, options.source_database,
                 ...
                 options.target_table, options.target_user, target_password, config, seg_sa2s,
                 options.printonly)
print("Done.")
