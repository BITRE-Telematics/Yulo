   ...
   # Open database onnection
    ...
    while True:
        ...
        for row in rows:
            ...
            segments_ = segment(config, row, seg_sa2s)
            segments += segments_
        ...
        try:
            query = """INSERT INTO %s (osm_id,class_id,source,target,length,reverse,
                maxspeed_forward,maxspeed_backward,priority,sa2,gcc,geom) VALUES %s;""" % (  ##add SA2, GCC between priority, geom
                tgt_table, ",".join("""('%s','%s','%s','%s','%s','%s', %s, %s,'%s','%s','%s',
...
def is_oneway(tags):
    if ("oneway" in tags.keys() and tags["oneway"] == "yes") \
        ...
        return False
##additional
def is_noaccess(tags):
    if ("access" in tags.keys() and tags["access"] == "private") \
            or ("access" in tags.keys() and tags["access"] == "no") \
            or ("access" in tags.keys() and tags["access"] == "bus"):
        return True
    else:
        return False

def is_wrong_service(tags):
    if ("service" in tags.keys() and tags["service"] == "parking_aisle"):
        return True
    else:
        return False

def is_no_motor_vehicles(tags):
    if ("motor_vehicle" in tags.keys() and tags["motor_vehicle"] == "no") \
            or ("motorcar" in tags.keys() and tags["motorcar"] == "no"):
        return True
    else:
        return False
...
##add seg_sa2s argument
def segment(config, row, seg_sa2s):
    segments = []
    ...
    if is_noaccess(tags):
        return segments
    if is_wrong_service(tags):
        return segments
    if is_no_motor_vehicles(tags):
        return segments
    osm_id = row[0] if isinstance(row[0], str) else str(row[0])
    ...
    for i in range(1, len(way[:, 0])):
        ...
        if ((int(way[i][2]) >= 2) or (i == (len(way[:, 0]) - 1))):
            ...
            geog[1],
            line.ExportToWkt()
            ) ##add Sa2, GCC between priority and line.Export
