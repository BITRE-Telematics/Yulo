...
then
	input=/mnt/map/osm/australia.osm.pbf
	database=australia
	user=osmuser
	password=pass
...
/opt/osmosis/package/bin/osmosis --read-pbf file=${input} --tf accept-ways highway=* route=ferry --write-pgsql user="${user}" password="${password}" database="${database}"
#/opt/osmosis/package/bin/osmosis --read-pbf file=${input} --tf accept-ways route=ferry --write-pgsql user="${user}" password="${password}" database="${database}"
echo "Done."
